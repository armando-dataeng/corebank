...
Este documento define las reglas de negocio que governan el

comportamiento de CoreBank. Constituyen la fuente de verdad del

dominio y cualquier decision de diseno posterior debe estar alineada

con ellas.



Las reglas estan agrupadas por area funcional. Cada regla tiene un

identificador numerico para facilitar referencias cruzadas en otros

documentos del proyecto (especialmente en los ADRs y en el modelo
...
1\. Todo cliente tiene un `cliente\_id` generado internamente, no reutilizable.

2\. El documento de identidad (cedula, RNC, pasaporte) es unico por tipo de documento.

3\. Un cliente es persona fisica o juridica, mediante discriminador `tipo\_cliente`.

4\. Los datos especificos de cada tipo (fecha de nacimiento para fisica, fecha de constitucion para juridica) se almacenan en tablas satelite.
...
7\. Un cliente tiene una sucursal de origen (donde fue registrado), que no cambia.
...
11\. Las reglas del producto (saldo minimo, limite de sobregiro) se copian en la cuenta al momento de apertura (snapshot).
...
14\. Una cuenta tiene una moneda (asumimos DOP en el MVP, campo presente para futuro).
...
24\. Los datos economicos y de identidad de una transaccion (`monto`, `cuenta`, `tipo`, `timestamp`, `transferencia\_id`) son inmutables desde el momento de su aplicacion.

25\. El campo `estado` puede transicionar de `APLICADA` a `REVERTIDA` exclusivamente como consecuencia de una reversion valida.

26\. Una reversion genera una nueva transaccion que referencia a la original mediante `transaccion\_revierte\_id`. La transaccion original no se modifica economicamente.
...
27\. Una transferencia tiene un `transferencia\_id` propio.

28\. Una transferencia genera exactamente dos transacciones (debito y credito) que comparten el `transferencia\_id`.

29\. Las dos transacciones se ejecutan en una sola transaccion de base de datos (atomicidad).

30\. Si alguna validacion falla (cuenta inexistente, bloqueada, saldo insuficiente, etc.), la transferencia falla completa sin generar transacciones.

31\. Los estados de una transferencia son `COMPLETADA`, `FALLIDA`, o `REVERTIDA`. No hay estado `PENDIENTE` en el MVP.

32\. Una transferencia revertida es una nueva transferencia que referencia a la original mediante `transferencia\_revierte\_id` y `es\_reversion = TRUE`.
...
36\. La auditoria de base de datos registra cambios a nivel de fila, incluso los directos.

37\. Ambas auditorias son append-only.

38\. La auditoria de BD registra los cambios realizados mediante las operaciones normales sobre las tablas auditadas y constituye la ultima capa de trazabilidad dentro de la base de datos. Los mecanismos de seguridad y separacion de privilegios deben impedir que usuarios operativos modifiquen o eliminen los registros de auditoria.
